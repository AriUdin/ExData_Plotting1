...
pwr.consm <- read.csv2("Electric_power_consumption.txt")
for (x in 3:9) {
...
GAP + ylab("Global Active Power (kilowatts)") 
dev.off()

png(filename = "Plot3.png", width = 480, height = 480, units = "px")
ESB + theme(legend.background = element_rect(linetype = "solid", colour = "black"))
dev.off()

png(filename = "Plot4.png", width = 480, height = 480, units = "px")
grid.arrange(GAP + ylab("Global Active Power"),
             Volt,
             ESB,
             GRP)
